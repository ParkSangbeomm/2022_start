...
def check_func():
    global init_count, count, pre_count
    
    announcement = driver.find_element_by_id("td_box23")
    announcement.click()
    time.sleep(1)
    new = driver.find_element_by_xpath('//*[@id="tr_box_23"]/table/tbody').find_elements_by_css_selector('img')
    s = len(new)
    if init_count==0:
        count,pre_count = s,s
    else:
    ...
    init_count += 1
    print((init_count-1)*5, "minute...")
    if(init_count==144):
        driver.quit()
    threading.Timer(300, check_func).start()
    
init_count, count, pre_count, login = 0,0,0,0
